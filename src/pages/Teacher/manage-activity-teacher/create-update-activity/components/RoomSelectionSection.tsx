...
const RoomSelectionSection: React.FC<Props> = ({
  ...
  const roomValidation = validateField('room_id', formData, validationMode, originalActivityStatus);
  const typeValidation = validateField('type', formData, validationMode, originalActivityStatus);
    ...
    <div className="flex flex-col space-y-4 mt-5">
      ...
      <div className="flex space-x-6 w-140">
        ...
        <div className="w-1/2">
          ...
          {/* ✅ แสดง validation error จาก validateField */}
          {roomValidation.hasError && (
            <FormHelperText error sx={{ mt: 1 }}>
              {roomValidation.helperText}
            </FormHelperText>
          )}
